...
{
  # One console password for both accounts, staged by the install from the
  # password store. SSH never uses it; it is for the day someone borrows a
  # display and a keyboard.
  users.users.root.hashedPasswordFile = "/persist/etc/passwords/console";
  users.users.${config.personal.userName}.hashedPasswordFile = "/persist/etc/passwords/console";
  ...
  environment.persistence."/persist" = {
    ...
    directories = [
      ...
      # Tailscale node identity. Without it every boot would need a fresh auth
      # key, and the one the install staged is single-use.
      "/var/lib/tailscale"
    ];
  };
}
