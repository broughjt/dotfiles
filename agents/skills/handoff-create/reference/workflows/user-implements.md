...
Write the findings to `.scratch/handoff-<project>/review-<slug>.md`, naming at
the top the commit they were made against, and describe them to the user as
well. If the session ends before the user signs them off, set `Review:
in-review, findings .scratch/handoff-<project>/review-<slug>.md`. If the user
signs them off in session, the field stays `none` and the log records the
acceptance.
