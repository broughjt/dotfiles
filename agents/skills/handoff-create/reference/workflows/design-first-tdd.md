...
2. **Plan document**: `.scratch/handoff-<project>/plan-<task>.md`, written so a
   *different* agent can implement from it without the design conversation's
   context.
...
4. **Implement**: a fresh agent reads the handoff doc and the plan doc, then
   ...
   in `Read first`. The plan doc moves to `.scratch/handoff-<project>/archive/`
   when the task lands.
...
| After | Field |
|---|---|
| the plan document | `in-review, plan .scratch/handoff-<project>/plan-<task>.md` |
