...
*** Lean
I got the specific fork of ~lean4-mode~ from a blogpost I found,
[[https://jakob.space/blog/what-ive-learned-about-formal-methods.html#fn.2]]. Definitely
worth looking the changes the fork has made, and maybe worth forking
myself for better understanding and control.

The hook for ~eglot-ensure~ also doesn't seem to be working upon first
entering a file, so I have to run ~eglot~ by hand. Probably needs
fixing somewhere.
